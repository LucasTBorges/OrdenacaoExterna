...
class Polifasica:
    ...
    def polifasear(self):
        while not self.completo:
            ...
            arquivos = [arq for arq in self.arquivos if not arq.isEmpty and arq != target_file]
            while (all([not arq.isEmpty for arq in arquivos])):
                ...
                for arq in self.arquivos:
                    if arq != target_file and len(arq.sequencias) > 0:
                        print('Popando arquivos...s')
                        arq.sequencias.pop(0)

                arqs = [arq.sequencias for arq in arquivos]
                print('arquivos: ' + str(arqs))
                arqs = [arq.sequencias for arq in self.arquivos]
                print('self.arquivos: ' + str(arqs))
